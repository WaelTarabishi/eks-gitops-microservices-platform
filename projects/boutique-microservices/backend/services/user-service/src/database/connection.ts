...
export const connectDB = async (): Promise<void> => {
  ...
  while (maxRetries === 0 || attempt < maxRetries) {
    ...
    try {
      ...
      pool = candidatePool;
      console.log(`Connected to PostgreSQL database for user service after ${attempt} attempt(s)`);
      return;
    } catch (error) {
      ...
      console.error(
        `Database connection attempt ${attempt} failed for user service. Retrying in ${delayMs}ms...`,
        error
      );
  ...
  throw new Error('Exceeded maximum PostgreSQL connection retries for user service');
};
